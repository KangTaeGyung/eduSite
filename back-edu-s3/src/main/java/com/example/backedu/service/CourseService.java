...
import com.example.backedu.dto.CourseDto;
...
//        기본키가 있으면 update
    ...
    public Optional<CourseDto> findById(String uuid) {

        //            findById : parameter 값 - ID, return  값 - Optional
        Optional<Course> courseOptional = coursesRepository.findById(uuid);

//        dto 변환
//        사용법 : modelMapper.createTypeMap(소스객체, DTO객체)
//                .addMappings(mapping -> {
//            mapping.map(source -> source.getter.getter2..., DTO객체::setter);
//        });
        TypeMap<Course, CourseDto> typeMap = modelMapper.getTypeMap(Course.class, CourseDto.class);

        if (typeMap == null) { // if not  already added
            modelMapper.createTypeMap(Course.class, CourseDto.class)
                    .addMappings(mapping -> {
                        mapping.map(source -> source.getCode().getCodeId(), CourseDto::setCode);
                    });
        }

        CourseDto courseDto = modelMapper.map(courseOptional.get(), CourseDto.class);

        Optional<CourseDto> courseDtoOptional = Optional.of(courseDto);
